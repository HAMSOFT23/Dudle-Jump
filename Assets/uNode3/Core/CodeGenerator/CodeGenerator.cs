		...
		private static string GetCorrectName(
			...
			switch(mData.targetType) {
				...
				case MemberData.TargetType.uNodeFunction: {
					...
					data += "(";
					{
						var func = mData.startItem.GetReference<FunctionRef>().reference;
						if(func == null) {
			...
			if(!object.ReferenceEquals(member, null)) {
				if(member.isAssigned) {
					if(member.isStatic) {
						...
						var ESV = member.startItem.GetReference<VariableRef>()?.reference;
						if(ESV != null) {
							return GetVariableName(ESV);
						}
					}
					else if(member.targetType == MemberData.TargetType.uNodeProperty) {
						var property = member.startItem.GetReference<PropertyRef>()?.reference;
						if(property != null) {
							return property.name;
						}
					}
					else if(member.targetType == MemberData.TargetType.uNodeLocalVariable) {
						var v = member.startItem.GetReference<VariableRef>()?.reference;
						if(v != null) {
							if(isInUngrouped) {
			...
			if(!object.ReferenceEquals(member, null)) {
				if(member.isTargeted) {
					...
					if(member.isStatic) {
						...
						var variable = member.startItem.GetReference<VariableRef>()?.reference;
						if(variable != null) {
		...
		public static string Value(object obj, string initializer = null, bool autoConvert = false, bool setVariable = false) {
			...
			if(ReflectionUtils.IsNullOrDefault(obj, type)) {
				...
				if(string.IsNullOrEmpty(initializer)) {
					...
					return New(typeof(Gradient), null, new[] { initializer });
				}
			}
			else {
