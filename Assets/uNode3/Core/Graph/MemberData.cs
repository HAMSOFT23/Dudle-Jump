	...
	public class MemberData : IGraphValue, IValueReference {
		...
		private object DoInvoke(GraphInstance graph, object[] paramValues) {
			...
			switch(targetType) {
				...
				case TargetType.uNodeProperty:
					...
					return (startTarget as IGraph).GetPropertyData(startName).Get(graph);
				case TargetType.Type:
					return startType;
				case TargetType.Values: {
					var obj = instance;
				...
				case TargetType.uNodeParameter:
					if(isDeepTarget) {
						return DoReflect(graph, ReflectionUtils.GetMemberTargetRef(memberInfo, graph.GetUserData(startItem.GetReferenceValue()), out _, paramValues), paramValues);
					}
					return graph.GetUserData(startItem.GetReferenceValue());
				case TargetType.uNodeGenericParameter:
					if(Items != null && Items.Length > 0) {
		...
		public void Set(GraphInstance graph, object value, object[] paramValues) {
			...
			switch(targetType) {
				...
				case TargetType.uNodeParameter: {
					if(isDeepTarget) {
						DoReflectSet(ReflectionUtils.GetMemberTargetRef(memberInfo, graph.GetUserData(startItem.GetReferenceValue()), out var parentTarget, paramValues), value, parentTarget);
					}
					else {
						graph.SetUserData(startItem.GetReferenceValue(), value);
					}
					return;
				}
				case TargetType.uNodeGenericParameter:
				...
				case TargetType.uNodeProperty:
					if(isDeepTarget) {
						...
						(startTarget as IGraph).GetPropertyData(startName).Set(graph, value);
					}
					break;
				case TargetType.uNodeLocalVariable:
				case TargetType.uNodeVariable: {
