		...
		public void EnsureInitialized() {
			...
			if (type != null) {
				...
				foreach(var v in target.GetAllVariables()) {
					...
					SetVariable(var.name, var.defaultValue);
				}
			}
			else {
				//Instance reflection graph
				Instance = RuntimeGraphUtility.InitializeObjectGraph(target, this);
				RuntimeGraphUtility.InitializeInstanceGraphValue(target, Instance, variables);
			}
		}
