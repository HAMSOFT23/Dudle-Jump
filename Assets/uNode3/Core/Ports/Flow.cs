...
namespace MaxyGames.UNode {
	public abstract class GraphRuntimeData {
		...
		public object GetUserData(object owner) => instance.GetUserData(owner);
		#endregion
	...
	public class StateFlow : Flow {
		...
		protected override void OnTrigger(FlowOutput output, out JumpStatement jump) {
			...
			if(flow != null) {
				if(!flow.IsFinished()) {
					throw new Exception("The target flow is a coroutine, therefore needed to triggerer with TriggerCoroutine or TriggerParallel.");
				}
	...
	public class CoroutineFlow : Flow {
		...
		protected override void OnTriggerParallel(FlowOutput output) {
			...
			if(targetFlow != null) {
				...
				var iterator = flow.GetIterator();
				if(iterator.MoveNext()) {
					throw new Exception("The target flow is a coroutine, therefore it is not supported to run with TriggerParallel.");
				}
			}
		}
