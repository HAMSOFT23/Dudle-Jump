...
namespace MaxyGames.UNode.Nodes {
    [EventMenu("GUI", "On Slider Value Changed")]
	[StateEvent]
	public class OnSliderValueChangedEvent : BaseComponentEvent {
		...
		public override void OnRuntimeInitialize(GraphInstance instance) {
			...
			if(val != null) {
				if(val is GameObject) {
					UEvent.Register<float>(UEventID.OnSliderValueChanged, val as GameObject, (value) => OnTriggered(instance, value));
				} else if(val is Component) {
					UEvent.Register<float>(UEventID.OnSliderValueChanged, val as Component, (value) => OnTriggered(instance, value));
				}
			} else {
				if(instance.target is Component comp) {
					UEvent.Register<float>(UEventID.OnSliderValueChanged, comp, (value) => OnTriggered(instance, value));
				} else {
