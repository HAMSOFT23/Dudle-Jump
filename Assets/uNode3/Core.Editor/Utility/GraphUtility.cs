...
namespace MaxyGames.UNode.Editors {
	public static class GraphUtility {
		...
		public static class CopyPaste {
			...
			public static UGraphElement[] Paste(UGraphElement parent) {
				return Paste(parent, data);
			}
			...
			private static UGraphElement[] Paste(UGraphElement parent, CopiedData data) {
				if(data == null)
				...
				//Do paste
				var result = DoPaste(parent, elements);
				//Clean-up data.
			...
			private static UGraphElement[] DoPaste(UGraphElement parent, UGraphElement[] elements) {
				...
				foreach(var element in allElements) {
					if(element is NodeObject nodeObject) {
						...
						//TODO: use recursive analizer for searching invalid references
						//throw new Exception($"Pasting {element.GetType()} is currently not implemented");
					}
				}
		...
		public static void SaveAllGraph() {
			...
			if(assetPath.Count > 0 && uNodePreference.preferenceData.autoBackupOnSave) {
				var dic = uNodePreference.backupPath + Path.DirectorySeparatorChar + "Graphs";
				...
				try {
					using(var zipToOpen = new FileStream(dic + Path.DirectorySeparatorChar + fileName, FileMode.OpenOrCreate)) {
						using(var archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update)) {
							foreach(var path in assetPath.Distinct()) {
								var name = Path.GetFileName(path);
								archive.CreateEntryFromFile(path, name, System.IO.Compression.CompressionLevel.NoCompression);
							}
						}
					}
					RemoveBackup();
					...
					Debug.LogError("Autobackup error: " + ex.ToString());
				}
			}
		}
