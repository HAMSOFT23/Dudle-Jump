...
namespace MaxyGames.UNode.Editors {
	public class BaseNodeView : UNodeView {
		...
		public override void ReloadView() {
			...
			#region Debug
			if(debugView == null && primaryInputFlow != null && (Application.isPlaying || GraphDebug.HasBreakpoint(nodeObject))) {
								...
								case StateType.Success:
									GUI.DrawTexture(layout, uNodeEditorUtility.MakeTexture(1, 1,
										Color.Lerp(
											...
											(GraphDebug.debugTime - nodeDebug.calledTime) * 2)
										)
									);
									break;
			...
			if(debugView != null) {
				this.ExecuteAndScheduleAction(() => {
					if(!this.IsVisible())
						return;
					bool hasBreakpoint = GraphDebug.HasBreakpoint(nodeObject);
					if(hasBreakpoint) {
