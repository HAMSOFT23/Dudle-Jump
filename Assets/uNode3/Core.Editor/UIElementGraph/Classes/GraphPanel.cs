	...
	internal class GraphPanel : VisualElement, IDisposable {
		...
		void InitializeView() {
			...
			{//Classes
				...
				plusElement.onClick = () => {
					var mPos = (plusElement.clickedEvent.currentTarget as VisualElement).GetScreenMousePosition((plusElement.clickedEvent as IMouseEvent).localMousePosition, graphEditor.window);
