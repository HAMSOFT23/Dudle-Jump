...
namespace MaxyGames.UNode.Editors.Commands {
	...
	class CreateMacroFlowInputPortCommand : PortMenuCommand {
		...
		public override bool IsValidPort(Node source, PortCommandData data) {
			if(data.portKind != PortKind.FlowInput)
				return false;
			return source.nodeObject.parent is NodeObject parent && parent.node is IMacro || source.nodeObject.graph is IMacroGraph;
		}
	}
	...
	class ToNodeInputPortCommand : PortMenuCommand {
		...
		public override bool IsValidPort(Node source, PortCommandData data) {
			if(data.portKind != PortKind.ValueInput)
				return false;
			var port = data.port as ValueInput;
			return filter.IsValidTypeForValueConstant(port.type) && port.type.IsByRef == false;
		}
	}
	...
	class PromoteToVariableInputPortCommand : PortMenuCommand {
		...
		public override void OnClick(Node source, PortCommandData data, Vector2 mousePosition) {
			...
			if(data.portType != null) {
				NodeEditorUtility.AddNewVariable(source.nodeObject.graph.variableContainer, "", data.portType.IsByRef ? data.portType.GetElementType() : data.portType, (variable) => {
	...
	class PromoteToLocalVariableInputPortCommand : PortMenuCommand {
		...
		public override void OnClick(Node source, PortCommandData data, Vector2 mousePosition) {
			...
			var port = data.port as ValueInput;
			if(data.portType != null) {
				NodeEditorUtility.AddNewVariable(graph.graphData.selectedRoot.variableContainer, "", data.portType.IsByRef ? data.portType.GetElementType() : data.portType, (variable) => {
					var m = port.defaultValue;
					if(m.isAssigned && !data.portType.IsByRef) {
						variable.defaultValue = m.Get(null);
					}
