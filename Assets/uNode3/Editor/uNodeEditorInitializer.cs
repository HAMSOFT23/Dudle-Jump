	...
	public class uNodeEditorInitializer {
		...
		static void Setup() {
			...
			if(GraphDebug.hasBreakpoint == null) {
				GraphDebug.hasBreakpoint = delegate (int graphID, int id) {
					if(nodeDebugData.TryGetValue(graphID, out var hash)) {
			...
			if(GraphDebug.addBreakpoint == null) {
				GraphDebug.addBreakpoint = delegate (int graphID, int id) {
					if(graphID == 0)
						return;
					if(!nodeDebugData.TryGetValue(graphID, out var hash)) {
						hash = new HashSet<int>();
						nodeDebugData[graphID] = hash;
					}
					if(!hash.Contains(id)) {
						hash.Add(id);
						SaveDebugData();
			...
			if(GraphDebug.removeBreakpoint == null) {
				GraphDebug.removeBreakpoint = delegate (int graphID, int id) {
					if(graphID == 0)
						return;
					if(!nodeDebugData.TryGetValue(graphID, out var hash)) {
						hash = new HashSet<int>();
